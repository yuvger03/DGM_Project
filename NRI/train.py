...
from utils import *
from modules import *
from transformer import PairwiseAttention
...
parser.add_argument('--suffix', type=str, default='_charged5',
...
if args.encoder == 'mlp':
    encoder = MLPEncoder(
        n_in=args.timesteps * args.embed_dim,  ### UPDATED
...
attn = PairwiseAttention(input_dim=args.dims, embed_dim=args.embed_dim, num_heads=4)

if args.load_folder and os.path.exists(os.path.join(args.load_folder, 'encoder.pt')):
...
def train(epoch, best_val_loss):
    ...
    for batch_idx, (data, relations) in enumerate(train_loader):
        ...
        data_attn = attn(data)  # [B, N, T, D]
        data_attn = data_attn.reshape(data.size(0), data.size(1), -1)  # [B, N, T*D]
        logits = encoder(data_attn, rel_rec, rel_send)
    ...
    for batch_idx, (data, relations) in enumerate(valid_loader):
        ...
        data_attn = attn(data)  # [B, N, T, embed_dim]
        data_attn = data_attn.reshape(data.size(0), data.size(1), -1)  # [B, N, T*embed_dim]
        logits = encoder(data_attn, rel_rec, rel_send)
...
def test():
    ...
    for batch_idx, (data, relations) in enumerate(test_loader):
        ...
        data_attn = attn(data_encoder)  # [B, N, T, embed_dim]
        data_attn = data_attn.reshape(data.size(0), data.size(1), -1)  # [B, N, T * embed_dim]
        logits = encoder(data_attn, rel_rec, rel_send)
        edges = gumbel_softmax(logits, tau=args.temp, hard=True)
